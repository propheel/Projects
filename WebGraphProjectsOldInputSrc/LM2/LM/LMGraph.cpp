...
bool LMGraph::loadFromTextFile(const char *fileName, const int h, const bool compress) {
	...
	while(fileLeft > 0) {
		if(fileLeft > readChunk)
			bytesRead = (int)fread(readBuffer, 1, readChunk, f);
		else
			bytesRead= (int)fread(readBuffer, 1, fileLeft, f);
		fileLeft -= bytesRead;
	...
	while(fileLeft > 0) {
		if(fileLeft > readChunk)
			bytesRead = (int)fread(readBuffer + readBufferStart, 1, readChunk, f);
		else {
			bytesRead= (int)fread(readBuffer + readBufferStart, 1, fileLeft, f);
			fclose(f);
		}
		fileLeft -= bytesRead;
