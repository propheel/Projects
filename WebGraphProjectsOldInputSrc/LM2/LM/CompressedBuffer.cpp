...
bool CompressedBuffer::uncompressBytes(unsigned char *outputB, long long &outSize, long long offset, long long len, bool compress) {
	if(compress) {
		z_stream strm;
		strm.zalloc = Z_NULL;
		strm.zfree = Z_NULL;
...
*/		outSize -= (long long)strm.avail_out;
		...
		memcpy(outputB, buffer + offset, len);
		return true;
	}
}