...
bool CStripeGraph::loadFromTextFile(const char *textFileName, const int tile_size) {
	...
	for(int i=0; i<tilesInARow; i++)
		tileRow[i].setSizes(tileSize, logTileSize);
...
//	printf(" tileSize: %d; stripSize: %d; tilesInARow: %d; memory reserved: %lld \n", tileSize, stripSize, tilesInARow, fileSize>>3);
	...
	tileForUncompress.setParams(tileSize, logTileSize, graphBuffer, outArr);
...
bool CStripeGraph::loadFromFile(const char *graphFileName, const char *offsetsFileName, const int tile_size) {
