...
*/	
		...
		for(int ts = 2048; ts >= 128; ts >>= 1) { // oryg 2048 - 128

				sprintf(setFileName, "D:\\webgraph\\sets\\%s.txt", sets[testSet]);
				printf("%s (%d): ", setFileName, ts);
				QueryPerformanceCounter(&t1);
				graph.loadFromTextFile(setFileName, ts);
				QueryPerformanceCounter(&t2);
				sprintf(graphFileName, "D:\\webgraph\\nostripes\\%d\\%s.g", ts, sets[testSet]);
				sprintf(offsetsFileName, "D:\\webgraph\\nostripes\\%d\\%s.o", ts, sets[testSet]);
				graph.saveToFile(graphFileName, offsetsFileName);
				printf("%.4lf s\n", (double)(t2.QuadPart - t1.QuadPart) / frequency.QuadPart);
/*				graph.loadFromFile(graphFileName, offsetsFileName, ts);
